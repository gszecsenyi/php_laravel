...
<div class="bg-white rounded-lg shadow-xl p-6">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">
        <i class="fas fa-calendar-plus text-pink-600"></i> Create New Booking
    </h1>
    ...
    <form action="{{ route('bookings.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <select name="apartment_house_id" id="apartment_house_id" 
                    ...
                    @foreach($apartmentHouses as $apartment)
                        <option value="{{ $apartment->id }}" data-price="{{ $apartment->price_per_night }}" {{ old('apartment_house_id') == $apartment->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="number" name="total_price" id="total_price" value="{{ old('total_price') }}" step="0.01" min="0"
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="notes" id="notes" rows="3" 
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-pink-500 focus:border-pink-500 @error('notes') border-red-500 @enderror">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
